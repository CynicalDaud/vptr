...
def init_models(img_channels, encC, encH, encW, dropout, out_layer, rpe, rank, Transformer_lr, resume_AE_ckpt, resume_Transformer_ckpt = None, num_encoder_layers = 12, num_past_frames = 10, 
    ...
    start_epoch, history_loss_dict = resume_training({'VPTR_Enc': VPTR_Enc, 'VPTR_Dec': VPTR_Dec}, {}, resume_AE_ckpt, map_location = f'cpu':{rank}')
    ...
    if init_Disc:
        ...
        if not train_Disc:
            _, _ = resume_training({'VPTR_Disc': VPTR_Disc}, {}, resume_AE_ckpt, map_location = f'cpu':{rank}')
    ...
    if resume_Transformer_ckpt is not None:
        start_epoch, history_loss_dict = resume_training({'VPTR_Transformer': VPTR_Transformer}, {'optimizer_T':optimizer_T}, resume_Transformer_ckpt, map_location = f'cpu':{rank}')
        loss_dict = init_loss_dict(loss_name_list, history_loss_dict)
