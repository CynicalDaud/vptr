...
if __name__ == '__main__':
    ...
    ckpt_save_dir = Path('./VPTR_chkpts/tfm_train')
    tensorboard_save_dir = Path('./VPTR_chkpts/tfm_train/vffp_tensorboard')
    resume_ckpt = None
    #resume_ckpt = Path('./VPTR_chkpts/tfm_train/epoch_2.tar')
    resume_AE_ckpt = Path('./VPTR_chkpts/Pre/cuda-AE-fs-e100.tar')
    #############Set the logger#########
    ...
    for epoch in range(start_epoch+1, start_epoch + epochs+1):
        ...
        with tqdm(enumerate(train_loader, 0), unit=" batch", total=len(train_loader)) as tepoch:
            for idx, sample in tepoch:
                              ...
                              #NAR_show_samples(VPTR_Enc, VPTR_Dec, VPTR_Transformer, sample, ckpt_save_dir.joinpath(f'Prog/test_gifs_epoch{epoch}_{idx}'))
                              sleep(0.1)
            ...
            if epoch % 10 == 0:
              save_ckpt({'VPTR_Transformer': VPTR_Transformer}, 
                        {'optimizer_T': optimizer_T}, 
                        epoch, loss_dict, '/content/drive/MyDrive/T_ckpt/')
              NAR_show_samples(VPTR_Enc, VPTR_Dec, VPTR_Transformer, sample, Path('/content/drive/MyDrive/T_ckpt/').joinpath(f'test_gifs_epoch{epoch}'))
